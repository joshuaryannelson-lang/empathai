...
const PAGES: PageSection[] = [
  // ── FOR EVERYONE ──
  {
    ...
    checks: [
      ...
      "Clicking each card highlights it and shows a description — look above the cards for the role description and 'Entering as' button",
  ...
  // ── FOR PATIENTS ──
  {
    ...
    checks: [
      ...
      "After joining, you're automatically taken to the profile setup page",
      "Shrink the window narrow — the form is centered and easy to use",
      "No random codes or technical text visible anywhere",
    ],
  },
  {
    id: "portal-profile-setup",
    name: "Profile Setup",
    url: "/portal/profile-setup",
    who: "Patients (first login only)",
    group: "For Patients",
    checks: [
      "After entering a join code, you are taken to the profile setup page (not straight to welcome)",
      "The page shows fields for preferred name, pronouns, and timezone",
      "Timezone is auto-detected from your browser",
      "Click 'Skip for now' — you are taken to the welcome page without entering any data",
      "Type an email address in the preferred name field — a warning appears saying to use first name only",
      "Type a phone number in the preferred name field — a warning appears saying to use first name only",
      "Fill in all fields and click 'Save & continue' — you are taken to the welcome page",
      "Visit /portal/profile-setup again after completing — you are redirected to welcome (not shown the form again)",
      "Shrink the window narrow — the form is centered and easy to use on mobile",
      "No last name, DOB, email, or phone fields appear anywhere on this page",
    ],
  },
  {
    id: "portal-checkin",
    ...
    group: "For Patients",
    checks: [
      "Open this page in a private/incognito window without joining first — you should be redirected to the join code page, not a blank screen",
  ...
  },
  {
    ...
    checks: [
      ...
      "Skip unless you have access to a patient with no check-in history — if no check-ins exist, a calm message should appear rather than an error or blank screen",
  ...
  },
  {
    ...
    checks: [
      ...
      "Skip unless you have access to a patient with no goals — if no goals exist, a calm message should appear rather than an error or blank screen",
  ...
  },
  {
    ...
    checks: [
      ...
      "In a private/incognito window, set your role to Therapist or Manager on the home page, then try navigating to /admin/dev directly in the URL bar — you should be redirected away from this page",
  ...
  {
    id: "demo-tour",
    name: "Demo Tour (Sprint I)",
    url: "/demo",
    who: "Anyone — start from the /demo page and pick a persona",
    group: "Demo Tour",
    checks: [
      "Overlay race condition: all 17 steps render spotlight + tooltip together (no step shows tooltip without cutout)",
      "Therapist step 1: Active Cases = 4, Avg Score = 5.8",
      "Therapist AI Briefing in demo: shows seeded data (Morgan/Jordan/Alex/Riley), no live API call fires",
      "AI Briefing header: single row (label + date + button inline, never wraps)",
      "Owner dashboard header: 'Your Organization' + practice count badge",
      "Manager dashboard header: practice name + 'Last 7 days'",
      "Therapist dashboard header: 'Your caseload' or therapist first name",
      "All three dashboards use AIBriefing.tsx (not ad-hoc text)",
      "/demo page: old 5-step tour section absent from DOM",
      "/demo page: 4 persona cards with updated CTA copy (Start practice manager tour, Start organization tour, etc.)",
      "PHI: no last names in any dashboard AI output or demo data",
      ...
      "Patient tour: 5 steps reach welcome → checkin → checkin submit → history → goals",
      "Practice owner tour: 3 steps reach /dashboard/manager?role=owner with correct spotlights",
      "Spotlight overlay CSS transition: clip-path 300ms ease, opacity 200ms ease",
      "Section labels on all dashboards use uppercase, #94a3b8 color",
    ],
  },
];
