...
export const demoTasks = [
  // Case 3 critical
  { id: "demo-task-03a", case_id: caseId(3), assignee: "therapist", title: "Follow up with patient before next session", description: "Patient's score dropped to 2 — check in via phone or message before the scheduled appointment.", status: "pending" as const, due_date: dateFuture(2), created_at: daysAgo(0), updated_at: daysAgo(0) },
  { id: "demo-task-03b", case_id: caseId(3), assignee: "patient", title: "Complete a brief mood check each morning", description: "Rate your mood 1-10 when you wake up. Bring the log to your next session.", status: "pending" as const, due_date: dateFuture(5), created_at: daysAgo(0), updated_at: daysAgo(0) },
  { id: "demo-task-03c", case_id: caseId(3), assignee: "therapist", title: "Review and update safety plan", description: "Patient's declining trajectory warrants a safety plan review in the next session.", status: "pending" as const, due_date: dateFuture(3), created_at: daysAgo(0), updated_at: daysAgo(0) },
  // Case 1
  { id: "demo-task-01a", case_id: caseId(1), assignee: "patient", title: "Practice the 4-7-8 breathing technique daily", description: "Inhale for 4, hold for 7, exhale for 8. Do this before bed.", status: "in_progress" as const, due_date: dateFuture(5), created_at: daysAgo(1), updated_at: daysAgo(0) },
  { id: "demo-task-01b", case_id: caseId(1), assignee: "therapist", title: "Send sleep hygiene resources", description: "Patient mentioned sleep improvements — reinforce with handout.", status: "completed" as const, due_date: dateAgo(1), created_at: daysAgo(3), updated_at: daysAgo(1) },
  // Case 5 — declining
  { id: "demo-task-05a", case_id: caseId(5), assignee: "therapist", title: "Explore relationship stressors in next session", description: "Patient mentioned relationship stress as primary concern this week.", status: "pending" as const, due_date: dateFuture(4), created_at: daysAgo(1), updated_at: daysAgo(1) },
  { id: "demo-task-05b", case_id: caseId(5), assignee: "patient", title: "Write down three things that went well today", description: "Practice positive reframing each evening this week.", status: "pending" as const, due_date: dateFuture(6), created_at: daysAgo(1), updated_at: daysAgo(1) },
  // Case 10 — declining
  { id: "demo-task-10a", case_id: caseId(10), assignee: "therapist", title: "Assess workplace burnout risk", description: "Patient scored 3 — explore burnout symptoms and coping options.", status: "pending" as const, due_date: dateFuture(3), created_at: daysAgo(1), updated_at: daysAgo(1) },
  // Case 8 — completed task
  { id: "demo-task-08a", case_id: caseId(8), assignee: "patient", title: "Complete exposure hierarchy worksheet", description: null, status: "completed" as const, due_date: dateAgo(2), created_at: daysAgo(7), updated_at: daysAgo(2) },
];
